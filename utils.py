...
def init_distributed_mode(args):
    ...
    setup_for_distributed(args.rank == 0)  # 屏蔽所有非rank0的输出  #TODO
...
def auto_load_model(args,
    ...
    if loss_scaler is not None:
        ...
        if args.auto_resume:
            ...
            # else:
            #     if args.resume:
            #         print("Auto resume checkpoint from best")
            #         _, client_states = model.load_checkpoint(args.resume, tag='checkpoint-best')
            #         # args.start_epoch = client_states['epoch'] + 1
            #         if model_ema is not None:
            #             if args.model_ema:
