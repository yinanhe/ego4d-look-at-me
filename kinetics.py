...
class SSRawFrameClsDataset(Dataset):
    ...
    def load_frame(self, sample, num_frames, sample_rate_scale=1):
        ...
        fname = os.path.join('s3://video_pub/Sth2Sthv1/',str(sample))
...
class VideoClsDataset(Dataset):
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1):
        ...
        # if not (os.path.exists(fname)): #TODO
        #     return []

        # avoid hanging issue #TODO
        ...
        try:
            if self.keep_aspect_ratio:
                if ('s3://' in fname):
                    ...
                    vr = VideoReader(memoryview(video_bytes), num_threads=1, ctx=cpu(0),mc=True)
                else:
                    vr = VideoReader(fname, num_threads=1, ctx=cpu(0))
            else:
                if ('s3://' in fname):
                    video_bytes = self.client.get(fname)
                    vr = VideoReader(memoryview(video_bytes), width=self.new_width, height=self.new_height,
                                     num_threads=1, ctx=cpu(0),mc=True)
                else:
    ...
    def __getitem__(self, index):
        ...
        while True: # TODO
            ...
            try:
                ...
                if self.video_loader:
                    ...
                    if('s3://' in video_name):
                        ...
                        # decord_vr = decord.VideoReader(memoryview(video_bytes), mc=True,num_threads=1) #default
                        decord_vr = decord.VideoReader(memoryview(video_bytes), ctx=cpu(0),mc=True) #TODO
                        # decord_vr = decord.VideoReader(video_bytes, ctx=cpu(0),mc=True) #TODO
                    else: # ���ش洢
                        decord_vr = decord.VideoReader(video_name, mc=False)
                    ...
                    # process_data = process_data.view(( -1 ,self.new_length) + process_data.size()[-2:]) #

                    # process_data= torch.transpose(process_data.view((self.new_length) + process_data.size()[1:]), 1, 2)
            ...
            if process_data is None:
                # index = random.randint(0, len(self.clips) - 1) #TODO
                index = index - 1
            else:
                break
